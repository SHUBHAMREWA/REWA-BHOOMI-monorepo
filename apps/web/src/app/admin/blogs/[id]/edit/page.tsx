...
import { Box, Typography, Button, TextField, Paper, CircularProgress, MenuItem, Grid, Alert, Accordion, AccordionSummary, AccordionDetails, FormControlLabel, Checkbox } from '@mui/material';
import ExpandMoreIcon from '@mui/icons-material/ExpandMore';
import { useRouter, useParams } from 'next/navigation';
import { useForm, Controller } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { UpdateBlogSchema, UpdateBlogInput } from '@rewa-bhoomi/validation';
import { useBlog, useUpdateBlog, useDeleteBlog } from '@/features/blogs/api/useBlogs';
...
export default function EditBlogPage() {
  ...
  const { control, handleSubmit, formState: { errors }, reset } = useForm<UpdateBlogInput>({
  ...
  const handleDelete = async () => {
    if (!window.confirm('Are you sure you want to delete this blog? All associated Cloudflare assets will also be deleted.')) return;
    try {
      setIsDeleting(true);
  ...
  if (fetchError || !blog) {
    return (
      <Box p={4}>
        <Alert severity="error">Failed to load blog details.</Alert>
      </Box>
    );
  }
