...
export default function PropertyPostingWizard({ propertyId }: { propertyId?: string }) {
  ...
  const PurposeIcon = ({ name }: { name: string }) => {
    if (name === 'Sell') return <SellIcon sx={{ fontSize: { xs: 22, sm: 34 }, color: '#2563EB' }} />;
    if (name === 'Key') return <KeyIcon sx={{ fontSize: { xs: 22, sm: 34 }, color: '#16A34A' }} />;
    if (name === 'Description') return <DescriptionIcon sx={{ fontSize: { xs: 22, sm: 34 }, color: '#D97706' }} />;
    if (name === 'Hotel') return <HotelIcon sx={{ fontSize: { xs: 22, sm: 34 }, color: '#9333EA' }} />;
    return <StoreIcon sx={{ fontSize: { xs: 22, sm: 34 }, color: '#0284C7' }} />;
  };

  const CategoryIcon = ({ name }: { name: string }) => {
    if (name === 'Home') return <HomeIcon sx={{ fontSize: { xs: 22, sm: 34 }, color: '#2563EB' }} />;
    if (name === 'Business') return <BusinessIcon sx={{ fontSize: { xs: 22, sm: 34 }, color: '#D97706' }} />;
    if (name === 'Landscape') return <LandscapeIcon sx={{ fontSize: { xs: 22, sm: 34 }, color: '#16A34A' }} />;
    return <MeetingRoomIcon sx={{ fontSize: { xs: 22, sm: 34 }, color: '#9333EA' }} />;
  };
  ...
  return (
    <Box sx={{ bgcolor: '#F8FAFC', minHeight: '100vh', py: { xs: 1.5, md: 4 } }}>
      <Container
        maxWidth="lg"
        sx={{
          px: { xs: 1.2, sm: 3 },
          // Clean & compact inputs on mobile devices
          '& .MuiInputBase-root': {
          ...
          '& .MuiInputLabel-root': {
            ...
            transform: { xs: 'translate(10px, 7px) scale(1)', sm: 'translate(14px, 9px) scale(1)' },
            '&.MuiInputLabel-shrink': {
              transform: 'translate(14px, -8.5px) scale(0.75)',
            },
          },
          '& .MuiSelect-select': {
            py: { xs: '6px !important', sm: '8.5px !important' },
          ...
          '& .MuiAutocomplete-inputRoot': {
            py: { xs: '2px !important', sm: '4px !important' },
            px: { xs: '6px !important', sm: '9px !important' },
          },
          '& .MuiAutocomplete-input': {
            py: { xs: '4px !important', sm: '6px !important' },
            px: { xs: '4px !important', sm: '6px !important' },
          },
          '& .MuiFormControl-root': {
            my: { xs: 0.2, sm: 0.4 },
          },
      ...
      >
        {/* Header Title */}
        <Box mb={{ xs: 1.5, sm: 3 }} textAlign="center">
          <Typography variant="h4" fontWeight={800} color="#0F172A" sx={{ fontSize: { xs: '1.25rem', sm: '2rem' } }}>
            Apni Property Post Karein
          </Typography>
          <Typography variant="body1" color="#64748B" mt={0.3} sx={{ fontSize: { xs: '0.75rem', sm: '0.95rem' } }}>
            Rewa ke #1 Real Estate Marketplace par asani se apni property list karein
          </Typography>
        </Box>

        {/* Stepper Header */}
        <Paper elevation={0} sx={{ p: { xs: 1, sm: 2.5 }, borderRadius: { xs: 2, sm: 3 }, mb: { xs: 1.5, sm: 3 }, border: '1px solid #E2E8F0', bgcolor: '#fff' }}>
          <Box sx={{ width: '100%', px: { xs: 0.5, sm: 0 } }}>
              ...
              <Step key={label} completed={activeStep > idx}>
                <StepLabel 
                  ...
                  sx={{ 
                    ...
                    '& .MuiStepLabel-label': { fontSize: { xs: '0.68rem', sm: '0.85rem' }, mt: { xs: 0.3, sm: 0.8 } }, 
                    '& .MuiStepIcon-root': { width: { xs: 18, sm: 24 }, height: { xs: 18, sm: 24 } } 
                  }}
                >{label}</StepLabel>
              </Step>
            ))}
        ...
        {((activeStep === 0)) && (
          <Box>
            {progressLevel > 0 && purpose ? (
              <Paper sx={{ p: { xs: 1, sm: 1.5 }, mb: { xs: 1.5, sm: 2.5 }, borderRadius: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: '#F8FAFC', border: '1px solid #E2E8F0' }}>
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600} textTransform="uppercase" letterSpacing={0.5} sx={{ fontSize: '0.62rem' }}>Listing Purpose</Typography>
                  <Typography variant="subtitle1" fontWeight={700} color="#0F172A" display="flex" alignItems="center" gap={0.5} sx={{ fontSize: { xs: '0.85rem', sm: '1rem' } }}><CheckCircleIcon sx={{ color: '#22c55e', fontSize: { xs: '1.05rem', sm: '1.25rem' } }} /> {LISTING_PURPOSES.find(p => p.key === purpose)?.title}</Typography>
                </Box>
                <Button size="small" variant="outlined" onClick={() => setProgressLevel(0)} sx={{ textTransform: 'none', borderRadius: 1.5, height: 26, px: 1, py: 0.2, fontSize: '0.72rem' }}>Change</Button>
              </Paper>
            ) : (
              <Box mb={{ xs: 2, sm: 4 }}>
                <Typography variant="h6" fontWeight={700} textAlign="center" mb={0.4} sx={{ fontSize: { xs: '0.95rem', sm: '1.35rem' } }}>
              Aap Kya Karna Chahte Hain?
            </Typography>
            <Typography variant="body2" color="text.secondary" textAlign="center" mb={{ xs: 1.5, sm: 2.5 }} sx={{ fontSize: { xs: '0.72rem', sm: '0.85rem' } }}>
                  ...
                  <Grid item xs={12} sm={6} md={4} key={item.key}>
                    <Paper
                      ...
                      sx={{
                        ...
                        '&:hover': { transform: 'translateY(-2px)', boxShadow: '0 8px 16px rgba(0,0,0,0.05)' },
                      }}
                    >
                      <Box display="flex" alignItems="center" gap={1} mb={0.5}>
                        <PurposeIcon name={item.iconName} />
                        <Typography variant="h6" fontWeight={700} color="#0F172A" sx={{ fontSize: { xs: '0.85rem', sm: '1.15rem' } }}>
                          {item.title}
                        </Typography>
                      </Box>
                      <Typography variant="body2" color="#64748B" sx={{ fontSize: { xs: '0.7rem', sm: '0.82rem' }, lineHeight: 1.3 }}>
                        {item.subtitle}
                      </Typography>
                    </Paper>
                  </Grid>
        ...
        {((activeStep === 0)) && progressLevel >= 1 && purpose !== null && (
          <Box>
            {progressLevel > 1 && category ? (
              <Paper sx={{ p: { xs: 1, sm: 1.5 }, mb: { xs: 1.5, sm: 2.5 }, borderRadius: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: '#F8FAFC', border: '1px solid #E2E8F0' }}>
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600} textTransform="uppercase" letterSpacing={0.5} sx={{ fontSize: '0.62rem' }}>Property Category</Typography>
                  <Typography variant="subtitle1" fontWeight={700} color="#0F172A" display="flex" alignItems="center" gap={0.5} sx={{ fontSize: { xs: '0.85rem', sm: '1rem' } }}><CheckCircleIcon sx={{ color: '#22c55e', fontSize: { xs: '1.05rem', sm: '1.25rem' } }} /> {PROPERTY_CATEGORIES.find(c => c.key === category)?.title}</Typography>
                </Box>
                <Button size="small" variant="outlined" onClick={() => setProgressLevel(1)} sx={{ textTransform: 'none', borderRadius: 1.5, height: 26, px: 1, py: 0.2, fontSize: '0.72rem' }}>Change</Button>
              </Paper>
            ) : (
              <Box mb={{ xs: 2, sm: 4 }}>
                <Typography variant="h6" fontWeight={700} textAlign="center" mb={0.4} sx={{ fontSize: { xs: '0.95rem', sm: '1.35rem' } }}>
                ...
                return (
                  <Grid item xs={6} sm={6} md={3} key={item.key}>
                    ...
                    >
                      ...
                      <Typography variant="h6" fontWeight={700} color="#0F172A" mb={0.3} sx={{ fontSize: { xs: '0.82rem', sm: '1.1rem' } }}>
                        {item.title}
                      </Typography>
                      <Typography variant="caption" color="#64748B" sx={{ fontSize: { xs: '0.68rem', sm: '0.75rem' }, lineHeight: 1.25, display: 'block' }}>
                        {item.subtitle}
                      </Typography>
                    </Paper>
                  </Grid>
        ...
        {((activeStep === 0)) && progressLevel >= 2 && category !== null && (
          <Box>
            {progressLevel > 2 && propertyType ? (
              <Paper sx={{ p: { xs: 1, sm: 1.5 }, mb: { xs: 1.5, sm: 2.5 }, borderRadius: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: '#F8FAFC', border: '1px solid #E2E8F0' }}>
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600} textTransform="uppercase" letterSpacing={0.5} sx={{ fontSize: '0.62rem' }}>Property Type</Typography>
                  <Typography variant="subtitle1" fontWeight={700} color="#0F172A" display="flex" alignItems="center" gap={0.5} sx={{ fontSize: { xs: '0.85rem', sm: '1rem' } }}><CheckCircleIcon sx={{ color: '#22c55e', fontSize: { xs: '1.05rem', sm: '1.25rem' } }} /> {filteredTypes.find(p => p.key === propertyType!)?.label}</Typography>
                </Box>
                <Button size="small" variant="outlined" onClick={() => setProgressLevel(2)} sx={{ textTransform: 'none', borderRadius: 1.5, height: 26, px: 1, py: 0.2, fontSize: '0.72rem' }}>Change</Button>
              </Paper>
            ) : (
              <Box mb={{ xs: 2, sm: 4 }}>
                <Typography variant="h6" fontWeight={700} textAlign="center" mb={0.4} sx={{ fontSize: { xs: '0.95rem', sm: '1.35rem' } }}>
            ...
            <Grid container spacing={{ xs: 0.8, sm: 1.5 }}>
              {filteredTypes.map((item) => {
                ...
                return (
                  <Grid item xs={6} sm={6} md={4} key={item.key}>
                    ...
                    >
                      <Typography variant="subtitle1" fontWeight={700} color="#0F172A" sx={{ fontSize: { xs: '0.8rem', sm: '0.95rem' } }}>
                        {item.label}
                      </Typography>
                      {isSelected && <CheckCircleIcon color="primary" sx={{ fontSize: { xs: '1rem', sm: '1.2rem' }, ml: 'auto' }} />}
                    </Paper>
                  </Grid>
                );
              })}
        ...
        {((activeStep === 1)) && (
          <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 3 }, mb: { xs: 1.5, sm: 3 }, borderRadius: { xs: 2, sm: 2.5 }, border: '1px solid #E2E8F0', bgcolor: '#fff' }}>
            <Typography variant="h6" fontWeight={700} mb={{ xs: 1.2, sm: 2.5 }} sx={{ fontSize: { xs: '0.95rem', sm: '1.25rem' } }}>
              Location Details
            </Typography>
            <Grid container spacing={{ xs: 1, sm: 2 }} mb={{ xs: 1.5, sm: 2.5 }}>
              ...
              <Grid item xs={12}>
                ...
                <FormControlLabel
                  ...
                  label={<Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.72rem' }}>Same as full address</Typography>}
                  sx={{ mt: 0.2, ml: 0 }}
                />
              </Grid>
            </Grid>
        ...
        {((activeStep === 0)) && progressLevel >= 3 && propertyType !== null && (
          <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 3 }, mb: { xs: 1.5, sm: 3 }, borderRadius: { xs: 2, sm: 2.5 }, border: '1px solid #E2E8F0', bgcolor: '#fff' }}>
            <Typography variant="h6" fontWeight={700} mb={{ xs: 1.5, sm: 2.5 }} sx={{ fontSize: { xs: '0.95rem', sm: '1.25rem' } }}>
            ...
            <Grid container spacing={{ xs: 1, sm: 2 }}>
              ...
              <Grid item xs={12} sm={6}>
                <FormControlLabel
                  ...
                  label={<Typography variant="body2" sx={{ fontSize: { xs: '0.78rem', sm: '0.88rem' }, fontWeight: 600 }}>Price Negotiable? (कीमत में गुंजाइश / मोलभाव संभव है?)</Typography>}
                  sx={{ mt: { xs: 0, sm: 0.5 } }}
                />
              </Grid>
        ...
        {((activeStep === 1)) && (
          <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 3 }, mb: { xs: 1.5, sm: 3 }, borderRadius: { xs: 2, sm: 2.5 }, border: '1px solid #E2E8F0', bgcolor: '#fff' }}>
            <Typography variant="h6" fontWeight={700} mb={0.4} sx={{ fontSize: { xs: '0.95rem', sm: '1.25rem' } }}>
              Property Ki Suvidhayein (Amenities) Select Karein
            </Typography>
            <Typography variant="body2" color="text.secondary" mb={{ xs: 1.5, sm: 2.5 }} sx={{ fontSize: { xs: '0.72rem', sm: '0.85rem' } }}>
              Aapki property me jo-jo suvidhayein uplabdh hain unhe tick karein
            </Typography>

              <Box display="flex" flexWrap="wrap" gap={{ xs: 0.6, sm: 1 }} mb={{ xs: 1.5, sm: 2.5 }}>
              ...
              <Typography variant="subtitle2" fontWeight={700} mb={0.4} sx={{ fontSize: { xs: '0.82rem', sm: '0.95rem' } }}>
                Extra Amenities (Upto 20)
              </Typography>
              <Typography variant="body2" color="text.secondary" mb={1.2} sx={{ fontSize: { xs: '0.7rem', sm: '0.82rem' } }}>
                Agar koi aur suvidha hai jo upar list me nahi hai, toh yahan type karke "Add" par click karein.
              </Typography>
              
              <Box display="flex" gap={0.8} mb={1.2}>
                <TextField
                  size="small"
                  ...
                  sx={{
                    flex: 1,
                    maxWidth: 280,
                    '& .MuiOutlinedInput-root': { height: { xs: 32, sm: 36 } },
                    '& .MuiOutlinedInput-input': { fontSize: '0.8rem', py: '5px' },
                  }}
                />
                <Button 
                  variant="contained" 
                  ...
                  sx={{ textTransform: 'none', px: 2, height: { xs: 32, sm: 36 }, fontSize: '0.78rem' }}
                >
                  Add
                </Button>
              </Box>
          ...
          <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 3 }, mb: { xs: 1.5, sm: 3 }, borderRadius: { xs: 2, sm: 2.5 }, border: '1px solid #E2E8F0', bgcolor: '#fff' }}>
            ...
            <Grid container spacing={{ xs: 1, sm: 2 }} mt={0.5}>
              <Grid item xs={6} sm={4} md={3}>
                <Box
                  ...
                  component="label"
                >
                  <AddIcon sx={{ fontSize: { xs: 26, sm: 40 }, color: '#64748B' }} />
                  <Typography variant="caption" fontWeight={600} color="#64748B" mt={0.5} sx={{ fontSize: { xs: '0.7rem', sm: '0.75rem' } }}>
                    Add Photo
                  </Typography>
              ...
              {imageUrls.map((url, idx) => (
                <Grid item xs={6} sm={4} md={3} key={url}>
                  <Card sx={{ position: 'relative', borderRadius: 2, height: { xs: 96, sm: 140 } }}>
                    <CardMedia component="img" height="140" image={url} alt={`Upload ${idx}`} sx={{ height: { xs: 96, sm: 140 }, objectFit: 'cover' }} />
                    <IconButton
                      size="small"
                      onClick={() => handleRemoveImage(idx)}
                      sx={{ position: 'absolute', top: 4, right: 4, bgcolor: 'rgba(0,0,0,0.6)', color: '#fff', p: 0.4, '&:hover': { bgcolor: '#EF4444' } }}
                    >
            ...
            {uploading && (
              <Box display="flex" alignItems="center" gap={1} mt={1.5}>
                ...
                <Typography variant="caption" sx={{ fontSize: '0.72rem' }}>Uploading to Cloudflare R2...</Typography>
              </Box>
            )}
          
            {/* Property Video */}
            <Box sx={{ mt: { xs: 2.5, sm: 4 } }}>
              <Typography variant="h6" fontWeight={700} mb={0.4} sx={{ fontSize: { xs: '0.9rem', sm: '1.15rem' } }}>
        ...
        {((activeStep === 0)) && progressLevel >= 3 && propertyType !== null && (
          <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 3 }, mb: { xs: 1.5, sm: 3 }, borderRadius: { xs: 2, sm: 2.5 }, border: '1px solid #E2E8F0', bgcolor: '#fff' }}>
            ...
            <Grid container spacing={{ xs: 1, sm: 2 }}>
              ...
              <Grid item xs={12}>
                <TextField
                  fullWidth multiline rows={3} label="Detailed Description" required
                  value={description}
        ...
        {activeStep === 2 && (
          <Box>
            <Box textAlign="center" mb={{ xs: 1.5, sm: 3 }}>
              <Typography variant="h5" fontWeight={800} color="#0F172A" mb={0.4} sx={{ fontSize: { xs: '1.05rem', sm: '1.5rem' } }}>
                🎯 Apni Listing Ka Preview Dekh Lo!
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ fontSize: { xs: '0.72rem', sm: '0.85rem' } }}>
                Publish karne se pehle sabhi details ek baar zaroor check kar lo — yahi exactly dikhega buyers ko.
              </Typography>
            </Box>

            <Alert severity="info" sx={{ mb: { xs: 1.5, sm: 3 }, borderRadius: 2, p: { xs: 0.8, sm: 1.5 }, fontSize: { xs: '0.75rem', sm: '0.85rem' } }}>
            ...
            {/* Main Info Card */}
            <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 3 }, borderRadius: { xs: 2, sm: 3 }, border: '1px solid #E2E8F0', bgcolor: '#fff', mb: { xs: 1.5, sm: 3 } }}>
              {/* Status Chips */}
              <Box display="flex" gap={0.8} mb={1.5} flexWrap="wrap">
                <Chip label={purpose} color="primary" size="small" sx={{ fontWeight: 700, fontSize: '0.72rem', height: 24 }} />
                <Chip label={category} variant="outlined" size="small" sx={{ fontSize: '0.72rem', height: 24 }} />
                <Chip label={(propertyType || "").replace(/_/g, ' ')} variant="outlined" size="small" sx={{ fontSize: '0.72rem', height: 24 }} />
                {!(user?.roles?.includes('ADMIN') || user?.roles?.includes('SUPER_ADMIN')) && (
                  <Chip label="⏳ Pending Admin Approval" size="small" sx={{ bgcolor: '#FEF3C7', color: '#92400E', fontWeight: 600, fontSize: '0.72rem', height: 24 }} />
                )}
              </Box>

              {/* Title */}
              <Typography variant="h5" fontWeight={800} color="#0F172A" mb={0.8} sx={{ lineHeight: 1.3, fontSize: { xs: '1.05rem', sm: '1.35rem' } }}>
              ...
              <Box display="flex" alignItems="center" gap={0.5} mb={1.5}>
                ...
                <Typography variant="body2" color="#64748B" sx={{ fontSize: { xs: '0.75rem', sm: '0.85rem' } }}>
              ...
              {/* Price */}
              <Box display="flex" alignItems="baseline" gap={1} mb={{ xs: 1.5, sm: 3 }}>
                <Typography variant="h4" fontWeight={800} color="#16A34A" sx={{ fontSize: { xs: '1.35rem', sm: '2.125rem' } }}>
                ...
                {isPriceNegotiable && (
                  <Chip label="Negotiable (मोलभाव संभव)" size="small" sx={{ bgcolor: '#DCFCE7', color: '#15803D', fontWeight: 700, border: '1px solid #86EFAC', fontSize: { xs: '0.68rem', sm: '0.75rem' }, height: 24 }} />
                )}
              </Box>
              ...
              {description && (
                <Box mb={{ xs: 1.5, sm: 3 }}>
                  <Typography variant="subtitle2" fontWeight={700} color="#475569" mb={1} sx={{ fontSize: { xs: '0.82rem', sm: '0.95rem' } }}>📝 Description</Typography>
                  <Typography variant="body2" color="#475569" sx={{ lineHeight: 1.6, whiteSpace: 'pre-line', fontSize: { xs: '0.78rem', sm: '0.875rem' } }}>
                    {description}
                  </Typography>
                </Box>
              )}
              ...
              {(selectedAmenityIds.length > 0 || customAmenities.length > 0) && (
                <Box mb={{ xs: 1.5, sm: 2 }}>
                  <Typography variant="subtitle2" fontWeight={700} color="#475569" mb={1} sx={{ fontSize: { xs: '0.82rem', sm: '0.95rem' } }}>✨ Suvidhayein (Amenities)</Typography>
                  <Box display="flex" gap={0.8} flexWrap="wrap">
                    {availableAmenities
                      .filter(a => selectedAmenityIds.includes(a.id))
                      .map(a => (
                        <Chip key={a.id} label={a.name} size="small" variant="outlined" sx={{ fontWeight: 500, fontSize: '0.72rem', height: 24 }} color="primary" />
                      ))
                    }
                    {customAmenities.map((amenity, idx) => (
                      <Chip key={`custom-${idx}`} label={amenity} size="small" variant="outlined" sx={{ fontWeight: 500, fontSize: '0.72rem', height: 24 }} color="secondary" />
                    ))}
                  </Box>
                </Box>
              )}
              ...
              {location.googleMapsLink && (
                <Box mt={{ xs: 1.5, sm: 2 }}>
                  <Chip
                    ...
                    clickable
                    sx={{ bgcolor: '#DBEAFE', color: '#1D4ED8', fontWeight: 600, fontSize: '0.75rem' }}
                  />
                </Box>
              )}
            </Paper>

            <Alert severity="warning" sx={{ borderRadius: 2, p: { xs: 0.8, sm: 1.5 }, fontSize: { xs: '0.75rem', sm: '0.85rem' } }}>
        ...
        {activeStep === 3 && (
          <Paper elevation={0} sx={{ p: { xs: 2.5, sm: 5 }, borderRadius: { xs: 2, sm: 3 }, border: '1px solid #E2E8F0', bgcolor: '#fff', textAlign: 'center' }}>
            <Box sx={{ fontSize: { xs: 48, sm: 72 }, mb: 1.5 }}>🎉</Box>
            <Typography variant="h4" fontWeight={800} color="#0F172A" mb={0.5} sx={{ fontSize: { xs: '1.25rem', sm: '1.75rem' } }}>
              {editingPropertyId ? 'Property Update Ho Gayi!' : 'Property Submit Ho Gayi!'}
            </Typography>
            <Typography variant="body1" color="#64748B" mb={1} sx={{ fontSize: { xs: '0.8rem', sm: '0.95rem' } }}>
              Aapki property successfully {editingPropertyId ? 'update' : 'submit'} ho gayi hai.
            </Typography>
            
            {user?.roles?.includes('ADMIN') || user?.roles?.includes('SUPER_ADMIN') ? (
              <Alert severity="success" sx={{ mb: { xs: 2.5, sm: 4 }, textAlign: 'left', borderRadius: 2, fontSize: { xs: '0.75rem', sm: '0.85rem' } }}>
              ...
              <Alert severity="info" sx={{ mb: { xs: 2.5, sm: 4 }, textAlign: 'left', borderRadius: 2, fontSize: { xs: '0.75rem', sm: '0.85rem' } }}>
            ...
            <Box display="flex" justifyContent="center" gap={1.5} flexWrap="wrap">
              <Button variant="contained" size="small" onClick={() => router.push('/properties')} sx={{ textTransform: 'none', px: { xs: 2.5, sm: 4 }, py: 0.8, fontSize: { xs: '0.82rem', sm: '0.9rem' } }}>
                Browse Properties
              </Button>
              <Button variant="outlined" size="small" onClick={() => { setActiveStep(0); }} sx={{ textTransform: 'none', px: { xs: 2.5, sm: 4 }, py: 0.8, fontSize: { xs: '0.82rem', sm: '0.9rem' } }}>
        ...
        {activeStep < 3 && (
          <Box display="flex" justifyContent="space-between" mt={{ xs: 2, sm: 3.5 }}>
            <Button
              size="small"
              disabled={activeStep === 0 || submitting}
              onClick={() => setActiveStep(prev => prev - 1)}
              startIcon={<ArrowBackIcon fontSize="small" />}
              sx={{ textTransform: 'none', px: { xs: 1.5, sm: 2.5 }, fontSize: { xs: '0.8rem', sm: '0.875rem' } }}
            >
              Back
            </Button>
            ...
            {activeStep < 2 ? (
              ...
              <Button
                ...
                disabled={submitting}
                sx={{ textTransform: 'none', px: { xs: 2.5, sm: 5 }, py: { xs: 0.8, sm: 1.2 }, fontWeight: 700, fontSize: { xs: '0.85rem', sm: '1rem' } }}
              >
                {submitting ? <CircularProgress size={20} color="inherit" /> : (propertyId ? 'Confirm & Update Property' : 'Confirm & Post Property')}
              </Button>
            )}
          </Box>
        )}
